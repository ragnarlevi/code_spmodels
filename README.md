...
The code requires the [tidyverse](https://www.tidyverse.org/) package. You can install it in R using:

```r
install.packages("tidyverse")
```

Before running any scripts, set your working directory to the folder containing the code and data files. For example:

```r
setwd("path/to/your/project")
```

To run the case study on a generated toy dataset run the code in `toy_data_analysis.R`.


The experiments described in the paper can be reproduced by running the following scripts:
...
**Note:** These scripts can take considerable time to complete. Thus we also include the simulation data and the plots in the paper can be generated by running the code in `visualize.Rvisualize.R`. This script will read the simulation results stored in the `data_sim` folder.

An example of how to run the models is provided below.
