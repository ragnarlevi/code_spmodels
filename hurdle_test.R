...
# Sccript to perform a test on hurdle models
...
do_hurdle_t <- function(density, id, calc_se){
  ...
  sim <- simulate_claims(50, 5000, spatial_type = "graph",additive =  additive, area = nr_regions, 
                         model_type = "hurdle", mixing = "ig", density = density,  seed = id)
  A <- sim$A
  ...
  for(t in ts){
    ...
    out_none <- hurdle(claims, X, locs, years,  agg_claims, 
    ...
    log_lik_p[[as.character(t)]] <- -out_none$optim_obj$value
    prop_p[[as.character(t)]] <- out_none$prop
    
    # Mixed Poisson
    a1 <- Sys.time()
    
    out_ig <- hurdle_mixed (claims, X, years, locs, agg_claims, A, exposure, 
    ...
    log_lik_ig[[as.character(t)]] <- out_ig$log_lik
    prop_ig[[as.character(t)]] <- out_ig$pi
    
    
    # Mixed Poisson known A
    out_ig_known <- hurdle_mixed (claims, X, years, locs, agg_claims, A, exposure, 
                               model_type = "learn_graph", additive = additive, mixing = "ig",  Emethod = "integration",
                               n_iter = 80, lambda = 0, optimizer_beta = "gd", optimizer_psi = "gd",
                               optimizer_a = "gd", optimizer_pi = "gd", optimizer_beta_phi = "gd", sgd = FALSE,
                               batch_size = 100, param_tol = 1e-4, Q_tol = 1e-6, verbose = 2, do_optim = FALSE, a_known = TRUE, calc_se =FALSE)
    ...
    log_lik_known_ig[[as.character(t)]] <- out_ig_known$log_lik
    prop_known_ig[[as.character(t)]] <- out_ig_known$pi
    
    
    save(
      list = ls(envir = environment()),      # all names in this env
      file =  paste0("sim_data/3hurdle_ig_per_t",round(density, 2), "id", id , ".RData" ),               # where to write
...
for(id in 1:10){
  print(id)
  if(id == 1){
    do_hurdle_t(0.8, id, TRUE)
    do_hurdle_t(0.4, id, TRUE)
  }else{
    do_hurdle_t(0.4, id, FALSE)
    do_hurdle_t(0.8, id, FALSE)
  }
}
...
source("hurdle_mixed.R")
source("simulate_data.R")
do_hurdle_lambda <- function(density, id, calc_se = FALSE){
  ...
  lambdas <- c(0, 0.1, 0.25, 0.5, 0.75, 1, 2.5, 5, 7, 10, 15, 20, 30)
  additive <- TRUE
  for(lambda in lambdas){
    for(t in c(10, 50, 100, 200)){
      ...
      time_p[[paste0(t, "-", lambda)]] <-  Sys.time() - a1
      ...
      log_lik_p[[paste0(t, "-", lambda)]] <- -out_none$optim_obj$value
      ...
      log_lik_ig[[paste0(t, "-", lambda)]]<- out_ig$log_lik
      
      
      save(
        list = ls(envir = environment()),      # all names in this env
        file =  paste0("sim_data/3hurdle_ig_per_t_lambda",round(density, 2), "id", id , ".RData" ),               # where to write
        envir = environment()                  # whose vars to save
      )
...
for(id in 1:10){
  do_hurdle_lambda(0.2, id, FALSE)
  do_hurdle_lambda(0.4, id, FALSE)
  do_hurdle_lambda(0.8, id, FALSE)
